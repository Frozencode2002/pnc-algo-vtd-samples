...
void sendDriverCtrl( int & sendSocket, const double & simTime, const unsigned int & simFrame )
{
  ...
  double accelTgtSpeed = ( 30.0 - ownSpeed ) / 7.0;   // default speed should be own preferred speed
  
  if ( haveSensorObject )
  {
    // let's go for the same speed as preceding vehicle: 
    if ( mNearestObject.ext.speed.x < -1.0e-2 )
      accelTgtSpeed = 1.5 * mNearestObject.ext.speed.x / 6.0;
    else
      accelTgtSpeed = 0.0;

    // let's go for a 2s distance
    double tgtDist = ownSpeed * 1.8;
  
    if ( tgtDist < 15.0 )   // minimum  distance to keep
        tgtDist = 15.0;
